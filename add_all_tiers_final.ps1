# Final comprehensive script to add ALL tier fields

$filePath = "C:\MQ2\lua\yalm2\config\armor_sets.lua"
$content = Get-Content $filePath -Raw

# Comprehensive tier mapping
$tierMap = @{
    ...
    'Obscured Gallant Resonance' = 50; 'Obscured Steadfast Resolve' = 50; 'Obscured Heroic Reflections' = 48
    'Fear Touched' = 1; 'Fear Stained' = 2; 'Fear Washed' = 3; 'Fear Infused' = 4
}

Write-Host "Starting with $($tierMap.Count) mapped entries"
    ...
    if ($line -match "display_name\s+=\s+""([^""]+)""") {
        ...
        if ($nextLine -notmatch "tier\s*=" -and $nextLine -notmatch "^\s*\}") {
            ...
            if ($setName) {
                ...
                # Pattern matching for Encrusted Clay (Underfoot components)
                if (!$tier -and $setName -match '^(Celestrium|Damascite|Vitallium|Palladium|Iridium|Rhodium|Stellite) Encrusted') {
                    $material = $matches[1]
                    if ($tierMap.ContainsKey($material)) {
...
Write-Host "Unmapped sets: $($unmapped.Count)"
if ($unmapped.Count -gt 0 -and $unmapped.Count -lt 50) {
