...
loader.filename = function(name, loot_type)
	return ("%s/yalm2/config/%s/%s.lua"):format(mq.luaDir, loot_type, name)
end
...
loader.load = function(rule, loot_type, reload)
	...
	local success, result = pcall(require, ("yalm2.config.%s.%s"):format(loot_type, rule.name))
	if not success then
		result = nil
		rule.failed = true
		Write.Warn("%s registration failed: %s", loot_type, rule.name)
		Write.Warn('To get more error output, you could try: "/lua run yalm2/config/%s/%s"', loot_type, rule.name)
	else
		if
