...
local actor = actors.register(function(message)
    ...
    elseif message.content.id == 'END_SCRIPT' then
        ...
        close_script()
    elseif message.content.id == 'YALM2_TEST' then
        --Handle test message from YALM2
        print(taskheader .. " Received test message from YALM2: " .. (message.content.message or "no message"))
        if debug_mode then
            print("Full message content:", message.content)
            print("Message sender:", message.sender and message.sender.character or "unknown")
        end
    elseif message.content.id == 'YALM_REQUEST_TASKS' then
        --Handle YALM_REQUEST_TASKS message, send task data to YALM2
        print(taskheader .. " Received YALM_REQUEST_TASKS from " .. (message.sender and message.sender.character or "unknown"))
        local my_tasks = get_tasks()
        local missing_data = get_missing_tasks()
        actor:send({ 
            id = 'YALM_TASK_DATA', 
            tasks = my_tasks,
            missing_tasks = missing_data
        })
    end
end)
...
local function main()
    ...
    while running do
        ...
        -- Check for YALM2 requests via global variables
        if _G.YALM2_NEEDS_TASKS and (_G.YALM2_REQUEST_TIMESTAMP or 0) > ((_G.TASKHUD_LAST_RESPONSE or 0) - 5) then
            print(taskheader .. " Detected YALM2 request via globals, sending task data...")
            
            -- Get current tasks and missing task data
            local my_tasks = get_tasks()
            local missing_data = get_missing_tasks()
            
            -- Store in global variables for YALM2 to read
            _G.TASKHUD_TASK_DATA = {
                tasks = my_tasks,
                missing_tasks = missing_data
            }
            _G.TASKHUD_DATA_TIMESTAMP = mq.gettime()
            _G.TASKHUD_LAST_RESPONSE = _G.YALM2_REQUEST_TIMESTAMP
            
            -- Clear the request flag
            _G.YALM2_NEEDS_TASKS = false
            
            print(taskheader .. " Task data provided to YALM2 via globals")
        end
    end
