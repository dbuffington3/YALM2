                ...
                elseif objective and objective.objective then
                    ...
                    if item_name_extracted then
                        ...
                        if objective_data and objective_data.item_name then
                            -- Already matched before - use the cached result
...
local function manual_refresh_with_messages(show_messages)
    ...
    Write.Info("MANUAL_REFRESH: Starting manual refresh with %d characters", #peer_list)
    local task_count = 0
    for _ in pairs(task_data.tasks or {}) do task_count = task_count + 1 end
    Write.Info("MANUAL_REFRESH: Processing %d characters with task data (plus ML's own tasks)", task_count)
    ...
    if task_data.my_tasks and #task_data.my_tasks > 0 then
        all_tasks[my_name] = task_data.my_tasks
        Write.Info("MANUAL_REFRESH: Added ML's own tasks: %d tasks for %s", #task_data.my_tasks, my_name)
    else
        Write.Info("MANUAL_REFRESH: ML's own tasks empty or nil (my_tasks=%s, count=%s)", 
    ...
    for character_name, tasks in pairs(task_data.tasks) do
        ...
        Write.Info("MANUAL_REFRESH: Added collector tasks: %d tasks for %s", #tasks, character_name)
    end
    Write.Info("MANUAL_REFRESH: all_tasks now has %d characters total", #all_tasks)
