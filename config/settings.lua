...
settings.get_char_settings_filename = function(character)
	...
	return ("%s/YALM2/yalm2-%s-%s.lua"):format(mq.configDir, mq.TLO.EverQuest.Server(), name)
end

settings.get_default_char_settings_filename = function()
	return ("%s/yalm2/config/defaults/char_settings.lua"):format(mq.luaDir)
end

settings.get_default_global_settings_filename = function()
	return ("%s/yalm2/config/defaults/global_settings.lua"):format(mq.luaDir)
end

settings.get_default_yalm_settings_filename = function()
	return ("%s/yalm2/config/defaults/yalm_settings.lua"):format(mq.luaDir)
end

settings.get_global_settings_filename = function()
	return ("%s/YALM2.lua"):format(mq.configDir)
end
...
settings.init_settings = function(character)
	assert(utils.make_dir(mq.configDir, "YALM2"))
